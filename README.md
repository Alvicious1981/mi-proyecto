...
- Herramientas administrativas backend: `system.descriptor`, `system.compatibility_report`, `system.backup_state`, `system.restore_state`, `audit.stats`, `audit.clear`.
...
- `frontend/app.js`: carga de artifacts reales (con fallback mock) para vista de servidor, capacidades y checks.
...
- **Datos reales** leyendo `artifacts/compatibility-report.json` y `artifacts/mcp-descriptor.json` (si se sirve el repo por HTTP).
- **Fallback mock** automático cuando no encuentra los artifacts.
...
Luego abre: `http://127.0.0.1:4173/frontend/index.html`.

> Nota: este frontend sigue siendo MVP y aún no invoca endpoints MCP en tiempo real.
